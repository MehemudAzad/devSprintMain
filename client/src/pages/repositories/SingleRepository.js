...
const SingleRepository = () => {
    ...
    const fetchCollaborationRequests = async () => {
      try {
        const response = await fetch(
          `http://localhost:5003/project/requests/${repository?.id}`
        );
        if (response.ok) {
    ...
    const approveCollaborationRequest = async (user_id) => {
      console.log('inside approve req ' , user_id);
        try{
            fetch('http://localhost:5003/project/invite/approve', {
            method: 'POST',
            headers: {
          ...
          .then(response => {
            if (response.ok) {
              const updatedRequests = collaborationRequests.filter(
...
WhiteBoard
                ...
                <div className="bg-indigo-950 p-2 rounded-xl">
                    ...
                    {/* Display collaboration requests */}
          {
            authFlag ? <>
          <div>
            <h2 className="text-3xl text-white mt-5">Collaboration Requests: </h2>
            {collaborationRequests.map((request) => (
              <div key={request.user_id} className="bg-gray-200 p-3 rounded-md mt-3">
                <p className="text-lg">User: {request.username}</p>
                <p className="text-lg">Email: {request.email}</p>
                <button className="btn btn-primary mt-2" onClick={() => approveCollaborationRequest(request.user_id)}>
                  Approve
                </button>
              </div>
            ))}
